...
export const Divider = styled.div<{
  ...
  ${props => {
    ...
    if (props.length) {
      return `width: ${props.length}${
        typeof props.length === 'number' ? 'px' : ''
      }; height:${props.thick != null ? props.thick : 1}px;`;
    }
  ...
  padding: ${props => (props.padding !== null ? props.padding : '0')};
  border-radius: ${props =>
    props.borderRadius != null ? props.borderRadius : 0}px;
`;
...
export const Spacer = styled.div<{ length: number; vertical?: boolean }>`
  ...
  ${props =>
    props.vertical
      ? `height:${props.length}px; width:1px`
      : `height:1px; width:${props.length}px`};
`;
...
export const MobileContainer = styled(Container)`
  @media only screen and ${props => props.theme.onlyMobile} {
    width: 375px;
    align-self: center;
  }
`;
